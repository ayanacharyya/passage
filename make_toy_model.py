...
if __name__ == "__main__":
    ...
    line_model_dict = {'nii':{'name':'N II', 'slope':-0.3, 'intercept':-19.0}, 'sii':{'name':'S II', 'slope':-0.3, 'intercept':-19.0}, 'ha':{'name':'H alpha', 'slope':-0.5, 'intercept':-18.5}} # dict for storing each line's name, slope (dex/length), intercept (dex)
    ...
    axes[0].fill_between(xarr, np.log10(num_profile * (1 - 1 / args.num_snr)), np.log10(num_profile * (1 + 1 / args.num_snr)), lw=0, alpha=alpha - 0.2, color=num_color)
    axes[0].fill_between(xarr, np.log10(den_profile * (1 - 1 / args.den_snr)), np.log10(den_profile * (1 + 1 / args.den_snr)), lw=0, alpha=alpha - 0.2, color=den_color)
    ...
    log_ratio_data = np.log10(num_data / den_data)

    # ------plot the noisy model-----------------------
    s, lw, alpha = 10, 0.5, 0.8
    axes[0].scatter(xarr, np.log10(num_data), s=s, lw=lw, alpha=alpha, c=num_color)
    axes[0].scatter(xarr, np.log10(den_data), s=s, lw=lw, alpha=alpha, c=den_color)
